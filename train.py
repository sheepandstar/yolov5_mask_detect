...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='yolov5s.pt', help='initial weights path') #初始化的权重文件的路径地址
    parser.add_argument('--cfg', type=str, default='models/yolov5s_mask.yaml', help='model.yaml path') #模型yaml文件的路径地址
    parser.add_argument('--data', type=str, default='data/mask.yaml', help='data.yaml path') #数据yaml文件的路径地址
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.yaml', help='hyperparameters path') #超参数文件路径地址
    parser.add_argument('--epochs', type=int, default=50) #训练轮次
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs') #喂入批次文件的多少
    parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640], help='[train, test] image sizes') #输入图片尺寸
    ...
    parser.add_argument('--exist-ok', action='store_true', help='existing project/name ok, do not increment') #模型目录是否存在，不存在就创建
